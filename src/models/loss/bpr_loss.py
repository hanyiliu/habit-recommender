...
def bpr_loss(
    ...
    Args:
        logits (torch.FloatTensor, (B, n_activities)): Unnormalized scores
        targets (torch.LongTensor, (B,)): True next activity indices
        n_neg (int, optional): Number of negative samples per positive. Defaults to 10.

    Returns:
        torch.FloatTensor: Scalar mean BPR loss.
    """
