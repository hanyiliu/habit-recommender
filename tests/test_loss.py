...
def test_combined_loss_is_cross_entropy_plus_lambda_align():
    # Fidelity term is softmax cross-entropy on the true next activity.
    logits, targets, routine = _inputs()
    lam = 0.5
    expected = F.cross_entropy(logits, targets) + lam * template_cross_entropy(logits, routine)
    got = combined_loss(logits, targets, routine, lambda_align=lam)
    assert torch.allclose(got, expected)


def test_combined_loss_lambda_zero_is_pure_cross_entropy():
    logits, targets, routine = _inputs(seed=1)
    got = combined_loss(logits, targets, routine, lambda_align=0.0)
